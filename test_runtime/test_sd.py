...
from case_data.sd import manager as sd_mgr

class Test_sd(Test_runtime_base): pass
# param_to_method_handler.data["start_timestamp"]="1629202650"  #因为load data  所以这句话放在setup里面是没用的，这句话可以便于使用历史数据调试而无需改动excel文档

data=cases.get_scaled_case_data('销售分销')
...
for i in data:
    ...
    setattr(Test_sd,item_name,sd_mgr.govern(i.get('full_case_path'),i.get('no')))#不知道为啥，用用例默认的path左斜杠会导致在setup class之前执行用例代码
